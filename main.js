...
function loadPrivateKeys() {
    try {
        ...
        if (privateKeys.length === 0) {
            ...
            // Do not exit here if running in a loop, let it retry
            return false;
        }
...
// Remove readline for automatic execution
...
// Global variable to store the count for automatic runs
let globalUploadCount = 1; // Default to 1 upload per wallet per cycle

async function runUploads(countPerWallet) {
    try {
        logger.banner();
        if (!loadPrivateKeys()) {
            logger.critical('No private keys loaded. Waiting for the next cycle.');
            return; // Skip execution if no keys
        }
        loadProxies();
        ...
        logger.summary(`Uploads per wallet: ${countPerWallet}`);
        ...
        // Ensure readline interface is closed if it was opened
        // if (rl) rl.close(); // Only close if it was initialized
        logger.bye('Process cycle completed.');
    }
}

async function startScript() {
    logger.banner(); // Show banner only once at startup

    // Initial question for how many files to upload per wallet
    const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
    });

    rl.question('How many files to upload per wallet per 24-hour cycle? ', async (count) => {
        const parsedCount = parseInt(count);
        if (isNaN(parsedCount) || parsedCount <= 0) {
            logger.error('Invalid number. Using default of 1 upload per wallet per cycle.');
            globalUploadCount = 1;
        } else {
            globalUploadCount = parsedCount;
        }
        rl.close(); // Close readline after getting input

        // Run immediately on start
        logger.info(`Starting first upload cycle with ${globalUploadCount} uploads per wallet.`);
        await runUploads(globalUploadCount);

        // Set interval for subsequent runs (24 hours = 24 * 60 * 60 * 1000 milliseconds)
        const twentyFourHours = 24 * 60 * 60 * 1000;
        logger.info(`Script will automatically restart in 24 hours (next run at ${new Date(Date.now() + twentyFourHours).toLocaleString('id-ID')}).`);
        setInterval(async () => {
            logger.section(`Starting new 24-hour cycle. Next run at ${new Date(Date.now() + twentyFourHours).toLocaleString('id-ID')}.`);
            await runUploads(globalUploadCount);
        }, twentyFourHours);
    });

    // Handle process exit for clean shutdown
    process.on('SIGINT', () => {
        logger.bye('Script received interrupt signal. Exiting gracefully...');
        if (rl && !rl.closed) rl.close(); // Ensure rl is closed if it's still open
        process.exit(0);
    });
}
